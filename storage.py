...
class S3StorageService:
    """Service to handle S3 storage operations using two buckets - general and vector"""
    ...
    def save_data(self, data: Dict, collection: str) -> Optional[str]:
        ...
        try:
            ...
            if collection == "profiles":
                ...
                # Fallback or raise error for unknown collection
                logger.warning(
                    f"Unknown collection type for S3 prefix: {collection}. "
                    f"Using root."
                )
                key_prefix = ""  # Or handle as an error
    ...
    def query_data(self, collection: str, query: Dict = None) -> List[Dict]:
        ...
        try:
            ...
            for obj in response.get("Contents", []):
                # Ensure we only process .json files if not querying
                # a specific ID
                if not obj["Key"].endswith(".json"):
                    continue

                data = json.loads(
                    self.s3.get_object(Bucket=self.general_bucket, Key=obj["Key"])[
                        "Body"
                    ].read()
                )
                if query:
    ...
    def update_data(self, collection: str, data_id: str, updates: Dict) -> bool:
        ...
        try:
            ...
            key = f"{key_prefix}{data_id}.json"
            self.s3.put_object(
                Bucket=self.general_bucket, Key=key, Body=json.dumps(current_data)
            )
            return True
        except Exception as e:
    ...
    def save_vector_embedding(self, vector_id: str, embedding: List[float]) -> bool:
        ...
        try:
            # Embeddings are stored in a flat structure
            key = f"embeddings/{vector_id}.json"
            self.s3.put_object(
    ...
    def get_all_data(self, collection: str) -> List[Dict]:
        """
        Get all data from a collection in S3 general bucket or local storage
        """
        return self.query_data(collection)
